...
class CountryNode(View):
    ...
    def post(self, request):
        ...
        if country_node_data:
            str_data = str(country_node_data, "utf-8").replace("'","\"")
            country_datas = str_data.split("|")
            for node in country_datas:
                if not node:
                    continue
                json_country_node = json.loads(node)
                hosts.append(json_country_node)
            return JsonResponse({"code": 200, "message": "success", "data": hosts})
        else:
            ...
            for host in hosts:
                ...
                if self.check_country_status(country, connect_data):
                    host['connect_data'] = ""
            ...
            if new_hosts:
                ...
                return JsonResponse({"code": 200, "message": "success", "data": new_hosts})
            else:
                return JsonResponse({"code": 200, "message": "success", "data": hosts})
