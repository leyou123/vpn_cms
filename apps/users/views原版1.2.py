...
class Register(View):
    ...
    def register(self, uuid, request, try_day, package_id, platform):
        ...
        expiration_time = int(time.time()) + 60 * 60 * 12
        print(try_day)
    ...
    def binding_device(self, uid, uuid, platform_id, package_id):
        ...
        # 数据库没有此设备要绑定
            ...
            if user.device_count >= user.max_device_count:  # 设备数最大值
                return JsonResponse(
                    {"code": 404, "message": "The devices have reached the max. Please sign out the others",
                     "data": user.get_info()})
            else:
                # 绑定设备
    ...
    def redis_check_device(self, datas, uuid, package_id, user_data, uid, platform_id):
        ...
        if device_count >= max_device_count:
            ...
            if result:
                ...
                return JsonResponse(
                    {"code": 404, "message": "The number of devices exceeds the upper limit", "data": datas})
        else:
            # db0.lpush("check_users", json.dumps(user_data))
            self.check_time(uid, platform_id)
...
class GetUserCode(View):
    ...
    def post(self, request):
        ...
        datas = json.loads(str(redis_data, "utf-8"))
        verify_code = datas.get("code", "")
        return JsonResponse({"code": 200, "message": "success", "data": {"code": verify_code}})
...
class Logout(View):
    def register1(self, uuid, request, try_day, package_id, platform):
        expiration_time = 0
        redis_data = db0.lpop("user_id")
        uid = int(str(redis_data, "utf-8"))
    ...
    #         try_day = user_config.temp_day
    #
    #     return self.register1(uuid, request, try_day, package_id, platform)
